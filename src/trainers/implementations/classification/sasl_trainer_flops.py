...
class SASLTrainerFLOPS(ExperimentTrainer):
    ...
    def _single_aggregated_warmup_epoch(self, device, server_model, server_optimizer, global_train_dataloader, client_models: dict, client_model_requires_any_grad, client_optimizers_main: dict, client_optimizers_ae: dict, max_nr_of_batches_in_epoch, epoch_nr, global_args):
        ...
        print("AGG Warmup FLOPS: ", (client_fp.get_total_flops()) / 1e9 / global_args['batch_size'])

        data = {"Client-side Warmup GFLOPS single batch all clients": (client_fp.get_total_flops()) / 1e9 / global_args['batch_size']}
                # "Server-side GFLOPS single batch all clients": (server_fp.get_total_flops() + server_bp.get_total_flops()) / 1e9,}

        json.dump(data, open(os.path.join(os.environ['MODEL_WEIGHTS_DIR'], "flops_stats_warmup.json"), "w"), indent=4)
    ...
    def _single_epoch(self, device, server_model, server_optimizer, global_train_dataloader, client_models: dict,
        ...
        client_fp = FlopCounterMode(display=False) if should_profile else contextlib.nullcontext()
        client_bp = FlopCounterMode(display=False) if should_profile else contextlib.nullcontext()
        server_fp = FlopCounterMode(display=False) if should_profile else contextlib.nullcontext()
        server_bp = FlopCounterMode(display=False) if should_profile else contextlib.nullcontext()


        for batch_nr, client_batch_dict in enumerate(tqdm(global_train_dataloader)):
            # Profiling Logic: Capture Batch 1 of Epoch 1 as the "representative" state
    ...
    def client_specific_alignment(self,
        ...
        # Hence the comm cost, is 2 times the size of the AE module
